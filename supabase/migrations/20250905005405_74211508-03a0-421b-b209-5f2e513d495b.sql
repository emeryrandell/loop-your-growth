...
-- Enable RLS for trainer_settings
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE tablename = 'trainer_settings' AND schemaname = 'public') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT FROM pg_tables WHERE tablename = 'subscriptions' AND schemaname = 'public') THEN
...
('finance', 5, 'Gratitude Budget', 'List 3 things you are grateful for that money cannot buy', 'Builds appreciation and reduces materialistic desires', '2', 10),
...
('recovery', 2, 'Gratitude Journal', 'Write 3 things you are grateful for before bed', 'Promotes positive thinking and better sleep', '1', 10),
...
('recovery', 5, 'Wind-Down Ritual', 'Create a 15-minute calming routine before bed', 'Signals your brain it is time to rest and recover', '3', 15);